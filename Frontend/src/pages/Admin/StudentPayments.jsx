...
const StudentPaymentInterface = () => {
  ...
  // Handle student ID input and fetch their courses
  ...
  const handlePaymentSubmit = async (e) => {
    ...
    if (!studentId || !newClass || !paymentAmount) {
    ...
    if (unpaidMonths.length > 0) {
      setErrorMessage('There are unpaid previous months. Please settle those payments first.');
    ...
    try {
      ...
      const response = await fetch('/api/payments/setStudentPaymentDetails', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
  ...
  const handleSettlePreviousPayment = async (paymentId) => {
    if (!paymentId) return;
    
    setIsLoading(true);
    
    try {
      const response = await fetch('/api/payments/setStudentPrevPaymentDetails', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${localStorage.getItem('token')}`
      ...
      // Update unpaid months
      const remainingUnpaid = updatedHistory.filter(payment => payment.payment_status === 'Not Paid');
      setUnpaidMonths(remainingUnpaid);
      
      setSuccessMessage('Previous payment settled successfully!');
    ...
    <div className="w-full mx-auto p-6 bg-gray-50 rounded-lg shadow-lg">
      ...
      {view === 'addPayment' && (
        <div className="bg-white p-6 rounded-lg shadow-sm">
          <h2 className="text-xl font-semibold mb-4 text-gray-700">Record Monthly Payment</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <select
                  ...
                  defaultValue="April"
                >
                  <option value={new Date().toLocaleString('default', { month: 'long' })}>current month</option>
                </select>
              </div>  
            ...
            {unpaidMonths.length > 0 && (
              <div className="mt-4 p-3 bg-amber-50 border border-amber-200 rounded-md">
                <p className="text-amber-700 flex items-center font-medium">
                  ...
                  There are {unpaidMonths.length} unpaid months
                </p>
                    ...
                    <tbody className="divide-y divide-gray-200">
                      {paymentHistory.map((payment) => (
                        <tr key={payment.pay_id} className={payment.payment_status === 'Not Paid' ? 'bg-red-50' : ''}>
                          ...
                          <td className="py-3 px-4 text-sm text-gray-900">{newClass}</td>
                          <td className="py-3 px-4 text-sm text-gray-900">Rs.{payment.fees}</td>
                          <td className="py-3 px-4 text-sm text-gray-900">{payment.payment_status === 'Not Paid'? payment.date:payment.time.split('T')[0]}</td>
                          ...
                          <td className="py-3 px-4 text-sm">
                            {payment.payment_status === 'Not Paid' ? (
                              <button
                                onClick={() => handleSettlePreviousPayment(payment.pay_id)}
